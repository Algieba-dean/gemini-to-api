...
Provide OpenAI- and Google-compatible structured output (JSON) on the chat and generate endpoints, so clients (including LangChain `with_structured_output`) receive directly-parseable JSON, implemented consistently across the Python server and the Cloudflare Worker.
